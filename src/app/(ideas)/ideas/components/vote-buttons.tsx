...
const VoteButtons = memo(({ 
  ...
  const { votes, userVotes, isLoading, handleVote, initialize, subscribeToChanges } = useVotesStore()
  ...
  useEffect(() => {
    initialize(ideaId, userId)
    const unsubscribe = subscribeToChanges(ideaId)
    return () => {
      unsubscribe()
    }
  }, [ideaId, userId, initialize, subscribeToChanges])
