...
UPDATE ideas
SET company = LEFT(company, 100)
WHERE char_length(company) > 100;

UPDATE ideas
SET author_name = LEFT(author_name, 50)
WHERE char_length(author_name) > 50;

-- Update tags to meet constraints
UPDATE ideas
SET tags = CASE 
    WHEN tags IS NULL THEN ARRAY['general']
    WHEN array_length(tags, 1) > 5 THEN (SELECT array_agg(tag) FROM (
        SELECT unnest(tags) as tag LIMIT 5
    ) t)
    ELSE tags
END;
...
ALTER TABLE ideas
ADD CONSTRAINT tags_length CHECK (array_length(tags, 1) <= 5),
...
-- Add constraint to ensure individual tag lengths
